...
@loader.tds
class FarmweatherMod(loader.Module):
    """Для автоматического фарминга коинов в ирисботе"""

    strings = {
        "name": "farmweather",
        "farmon": "<i>✅Отложенка создана, автопогода запущена, всё начнётся через 20 секунд...</i>",
        "farmon_already": "<i>Уже запущено</i>",
        "farmoff": "<i>❌Автопогода остановлен.\nОтправлено:</i> <b>%coins% i¢</b>",
        "farm": "<i>Отправлено:</i> <b>%coins% i¢</b>",
    }
    ...
    async def farmcmd(self, message):
        """Вывод кол-ва погод, добытых этим модулем"""
        coins = self.db.get(self.name, "coins", 0)
    ...
    async def watcher(self, event):
        ...
        if not status:
            return
        if event.raw_text == ".w Смела":
            return await self.client.send_message(
                self.iris, ".w Смела", schedule=timedelta(minutes=random.randint(1, 20))
            )
        ...
        if "НЕЗАЧЁТ!" in event.raw_text:
            ...
            return await self.client.send_message(self.iris, ".w Смела", schedule=delta)
